...
DIR_DAY_N='day06'                        # folder name
FILENAME_RTL_TOP='shiftreg_top'         # file name rtl top module
FILENAME_TB_TOP='tb_shiftreg_top'       # file name tb top
FILENAME_VVP='shiftreg_top'             # file name output vvp
FILENAME_SYNTH='day06_synth'             # file name yosys sim script
FILENAME_NETLIST='netlist_shiftreg_top' # file name yosys generated netlist
...
if [[ $opt == 'Y' ]] || [[ $opt == 'y' ]]
then
    echo -e "\n-------------------------- Synthesizing Day 1 RTL --------------------------"

    touch ${PATH_SYNTH_FILE}
    # ----- create script for yosys sim
    # read design file
    echo "read -sv ${DIR_DAY_N}/${FILENAME_RTL_TOP}.sv" >> ${PATH_SYNTH_FILE}
    echo "# read -sv day6/shiftreg_pipo.sv" >> ${PATH_SYNTH_FILE}
    echo "# read -sv day6/shiftreg_piso.sv" >> ${PATH_SYNTH_FILE}
    echo "# read -sv day6/shiftreg_sipo.sv" >> ${PATH_SYNTH_FILE}
    echo "# read -sv day6/shiftreg_siso.sv" >> ${PATH_SYNTH_FILE}
